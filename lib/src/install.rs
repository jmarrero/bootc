    ...
    if has_ostree && root_setup.boot.is_some() {
        if let Some(boot) = &root_setup.boot {
            ...
            crate::task::Task::new_and_run(
                format!("Mounting {source_boot} to {target_boot}"),
                "mount",
                [source_boot.as_str(), target_boot.as_str()],
            )?;
        }
    }
...
pub(crate) struct RootSetup {
    ...
    /// Target root path (e.g., /target for to-filesystem installs).
...
async fn install_with_sysroot(
    ...
    if cfg!(target_arch = "s390x") {
        ...
        // Use target_root_path if available (e.g., for to-filesystem installs on ostree OS),
        // otherwise fall back to physical_root_path
        let bootupd_root = rootfs
            .target_root_path
            .clone()
            .unwrap_or_else(|| rootfs.physical_root_path.clone());
