...
fn read_staged_bootconfig_extra_sources() -> Result<BTreeMap<String, CmdlineOwned>> {
    let mut sources = BTreeMap::new();
    let staged_path = std::path::Path::new("/run/ostree/staged-deployment");
    if !staged_path.exists() {
        return Ok(sources);
    }

    let data = std::fs::read(staged_path).context("Reading staged deployment data")?;
    ...
    if let Some(extra) = dict.lookup_value("bootconfig-extra", None) {
        ...
        if inner.type_().as_str() == "a{ss}" {
            for i in 0..inner.n_children() {
                ...
                let value: String = entry.child_value(1).get().unwrap_or_default();
...
pub(crate) fn set_options_for_source_staged(
    ...
    let source_options = if staged.is_some() {
        ...
        let staged_sources = read_staged_bootconfig_extra_sources()?;
